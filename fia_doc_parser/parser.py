...
from fia_doc_parser._constant import QUALI_DRIVERS
from fia_doc_parser.models.classification import SessionEntryImport, SessionEntryObject
from fia_doc_parser.models.driver import RoundEntryImport, RoundEntryObject
from fia_doc_parser.models.foreign_key import (
    PitStopForeignKeys,
    RoundEntry,
    SessionEntryForeignKeys,
)
from fia_doc_parser.models.lap import LapImport, LapObject
from fia_doc_parser.models.pit_stop import PitStopData, PitStopObject
from fia_doc_parser.utils import (
    Page,
...
WHITE_STRIP_MIN_HEIGHT = (
    10  # A table should end with a white strip with at least 10px height
)
LINE_MIN_VGAP = (
    5  # If two horizontal lines are vertically separated by less than 5px, they are
)
                    ...
                    if len(cell) > 1:
                        ...
                        if cell[4].strip():
                            ...
                            if (
                                ...
                                raise ValueError(
                                    f"Found text outside the cell's area in row {i}, "
                                    f"col. {j} on p.{page.number} in {file}: {cell}"
                                )
                            text = cell[4].strip()
                    else:
                        raise ValueError(
                            f"Expected one text block in row {i}, col. {j} on "
...
class EntryListParser:
    def __init__(self, file: str | os.PathLike, year: int, round_no: int):
        self.file = file
        self.year = year
    ...
    def _parse_table_by_grid(
        ...
        for i in range(len(hlines) - 1):
            ...
            for j in range(len(vlines) - 1):
                ...
                if (
                    len(spans) == 1
                ):  # Only one text, which is the usual text, and no superscript
                    row.append(spans[0]["text"].strip())
                elif (
                    len(spans) == 2
                ):  # Two texts. Should be one usual text and one superscript  # noqa: E501, PLR2004
                    n_superscripts = 0
                    n_regular_text = 0
                    for span in spans:
                        if span["flags"] == 0:
                    ...
                    if n_superscripts == 2 or n_regular_text == 2:  # noqa: PLR2004
                        ...
                        if (spans[0]["size"] - spans[1]["size"]) / temp > 0.2:  # noqa: PLR2004
                            ...
                            raise ValueError(
                                f"Cannot determine which text is superscript in "
                    ...
                    raise ValueError(
                        f"Found more than two text blocks in the cell in row {i}, "
            ...
            if len(superscripts) > 1:
                raise ValueError(
                    f"Found multiple superscripts in row {i} on p.{page.number} in "
                    f"{self.file}. Expected one or no superscript per row. Found: "
                    f"{superscripts}"
                )
            if (
                row
            ):  # Empty iff. we are at the gap between the main and reserve driver table
                row.append(
                    superscripts[0] if superscripts else None
                )  # Add superscript to the row
                cells.append(row)
        ...
        if df.shape[1] == 5:  # noqa: PLR2004
            ...
            raise ValueError(
                f"Expected either 5 or 6 cols. in {self.file}. Found: "
                f"{df.columns.tolist()}"
            )
        df.car_no = df.car_no.astype(int)
        assert df.car_no.is_unique, (
            f"Car No. is not unique in {self.file}: {df.car_no.tolist()}"
        )
        ...
        for i in df.reserve.dropna().unique():  # For each superscript
            ...
            if len(temp) == 2:  # noqa: PLR2004
                ...
                df.loc[df[df.reserve == i].index[1], "reserve_for"] = temp.car_no.iloc[
                    0
                ]
    ...
    def _parse(self) -> pd.DataFrame:
        ...
        for page in doc:
            ...
            if (
                ...
                and "Nat" in text
                and "Team" in text
                and "Constructor" in text
            ):
                found = True
                break
        if not found:
            raise ValueError(
                f"Could not find any page containing entry list table in {self.file}"
            )
        ...
        for col in ["No.", "Driver", "Nat", "Team", "Constructor"]:
            temp = page.search_for(col, clip=(0, driver.y0, page.bound()[2], driver.y1))
            assert len(temp) == 1, (
                f'Expected one "{col}", got {len(temp)} in {self.file}'
            )
        ...
        """
        line_gap = sorted(
            [abs(car_nos[i + 1][1] - car_nos[i][3]) for i in range(len(car_nos) - 1)]
        )[-2]
        """
                ...
                try:
                    drivers.append(
                        RoundEntryImport(
                            ...
                            objects=[RoundEntryObject(car_number=x.car_no)],
                        ).model_dump(exclude_unset=True)
                    )
                except ValidationError as e:
                    warnings.warn(
                        f"Error when parsing driver {x.driver} in {self.file}: {e}"
                    )
            return drivers
...
class PracticeParser(BaseParser):
    def __init__(
        ...
        self.session = (
            "fp1" if session == "fp" else session
        )  # Sprint weekend FP renamed to FP1
        self.year = year
        self.round_no = round_no
        self._check_session()
    ...
    def _parse_classification(self) -> pd.DataFrame:
        ...
        if not found:
            doc.close()
            raise ValueError(
                f'"Practice Session Classification" not found on any page in '
        ...
        for i, is_white in enumerate(is_white_row):
            if is_white and strip_start is None:
                ...
                if (
                    i - strip_start >= WHITE_STRIP_MIN_HEIGHT
                ):  # At least 10 rows of white pixels
                    white_strips.append(strip_start + t)
                strip_start = None
        if (
            strip_start is not None
            and len(is_white_row) - strip_start >= WHITE_STRIP_MIN_HEIGHT
        ):
            white_strips.append(strip_start + t)
        if not white_strips:
            raise ValueError(
                f"Could not find a white strip below the table on p.{page.number} in "
                f"{self.classification_file}"
            )
        ...
        vlines = [
            ...
            1.5 * pos["TIME"]["left"]
            - 0.5 * pos["TIME"]["right"],  # Left of "TIME" minus the
        ...
        car_nos = page.get_text(
            "blocks", clip=(pos["NO"]["left"], y, pos["NO"]["right"], b)
        )
        hlines = [y]
...
class RaceParser(BaseParser):
    ...
    def is_pdf_complete(self) -> bool:
        ...
        if (
            self.lap_analysis_file is None
            or self.history_chart_file is None
    ...
    def starting_grid(self) -> pd.DataFrame:
        ...
        if self.is_pdf_complete is False:
            raise FileNotFoundError(
                "Lap chart, history chart, or lap time PDFs is missing. Can't "
    ...
    def _parse_classification(self) -> pd.DataFrame:
        ...
        for i in range(len(doc)):
            ...
            if found:
                warnings.warn(
                    "Found and using provisional classification, not the final one"
                )
                break
            else:
                found = page.get_image_header()
        ...
        if not found:
            doc.close()
            raise ValueError(
                f'"Final Classification" or "Provisional Classification" not found '
        ...
        if not bottom:
            raise ValueError(
                f'Could not find "NOT CLASSIFIED" or "FASTEST LAP" in '
                f"{self.classification_file}"
            )
        b = bottom[0].y0
        ...
        if has_not_classified:
            ...
            car_nos = []
            while car_no := page.get_text(
                "blocks", clip=(vlines[1], t, vlines[2], t + line_height)
            ):
                assert len(car_no) == 1, (
                    f'Error in detecting rows in the "NOT CLASSIFIED" '
        ...
        if disqualified:
            disqualified.sort(
                key=lambda x: x.y0
            )  # Can have multiple "DISQUALIFIED" on one page
            ...
            if np.isclose((disqualified.x0 + disqualified.x1) / 2, w / 2, rtol=0.1):
                ...
                pixmap = np.ndarray(
                    [b - t, r - l, 3], dtype=np.uint8, buffer=pixmap.samples
                )
                is_white_row = np.all(pixmap == 255, axis=(1, 2))  # noqa: PLR2004
                white_strips = []
                strip_start = None
                ...
                if not white_strips:
                    raise ValueError(
                        f'Could not find a white strip below the "DISQUALIFIED" '
                        f"table in {self.classification_file}"
                    )
                b = white_strips[0]
    ...
    def _parse_history_chart(self) -> pd.DataFrame:
        ...
        for page in doc:
            ...
            for i, header in enumerate(headers):
                ...
                header_text = page.get_text(
                    "text", clip=(left_boundary, header.y0, right_boundary, header.y1)
                )
                lap_no = re.search(r"LAP (\d+)", header_text)
                assert lap_no, (
                    f"Cannot find lap No. in header of table {i} on p.{page.number} "
                    f"in {self.history_chart_file}"
                )
                lap_no = int(lap_no.group(1))

                # Get the table content below header
                temp = page.find_tables(
                    clip=(left_boundary, top_boundary, right_boundary, h),
                    strategy="lines",
                ...
                if any([i.isdigit() for i in temp.columns[2]]):
                    ...
                    temp.iloc[-1, 1] = (
                        ""  # "GAP" col. is always empty for the race leader
                    )
                    temp.index += 1
                ...
                temp = temp[
                    temp.car_no != ""
                ]  # Sometimes will get one additional empty row
        ...
        df.loc[
            (df.car_no == df.car_no.shift(-1)) & (df.lap == df.lap.shift(-1)), "lap"
        ] -= 1
        df.loc[
            (df.car_no == df.car_no.shift(1)) & (df.lap == df.lap.shift(1) + 2), "lap"
        ] -= 1
        del df["index"]
    ...
    def _parse_lap_chart(self) -> pd.DataFrame:
        ...
        df = []
        for page in doc:
            t = page.search_for("Lap Chart")[
                0
            ].y1  # The table is below "Race Lap Chart"
            ...
            if b:  # The table is above "page x of y"
                ...
                if b:
                    ...
                    raise ValueError(
                        f"Cannot find © on p. {page.number} in {self.lap_chart_file}"
                    )
            ...
            positions = page.get_text(
                "dict", clip=(pos.x1, pos.y0, page.bound()[2], pos.y1)
            )
            assert len(positions["blocks"]) == 1, (
                f"Error in parsing positions in top row on "
            ...
            tab, superscript, cross_out = page.parse_table_by_grid(
                vlines=[
                    (col_seps[i], col_seps[i + 1]) for i in range(len(col_seps) - 1)
                ],
                hlines=[
                    (row_seps[i], row_seps[i + 1]) for i in range(len(row_seps) - 1)
                ],
            )
            ...
            if (tab.POS == "GRID").any():
                ...
                self.starting_grid.starting_grid = (
                    self.starting_grid.starting_grid.astype(int)
                )
    ...
    def _parse_lap_analysis(self) -> pd.DataFrame:
        ...
        for page in doc:
            ...
            if self.session == "race":
                ...
                for i in range(len(laps) - 1):
                    assert np.isclose(
                        laps[i].y1, laps[i + 1].y1, atol=1
                    ) and np.isclose(times[i].y1, times[i + 1].y1, atol=1), (
                ...
                for i in range(0, len(laps) - 1, 6):
                    for j in range(6):
                        if i + j == len(laps):
                            break
                        assert np.isclose(
                            laps[i + j].y0, laps[i].y0, atol=1
                        ) and np.isclose(times[i + j].y0, times[i].y0, atol=1), (
            ...
            if self.session == "race":
                ...
                for i in range(3):
                    ...
                    lines = [
                        i
                        for i in page.get_drawings_in_bbox((l, t, r, t + 10))
                        if np.isclose(i["rect"].y0, i["rect"].y1, atol=1)
                    ]
                    assert len(lines) >= 1, (
                        f"Expected at least one horizontal line for "
                        f"table(s) in col. {i} in page {page.number} in "
                        f"{self.history_chart_file}. Found none"
                    )
                    assert np.allclose(
                        [i["rect"].y0 for i in lines], lines[0]["rect"].y0, atol=1
                    ), (
                    ...
                    for line in top_lines:
                        no = page.search_for(
                            "LAP", clip=(line[0], line[1] - 15, line[2], line[3])
                        )
                        assert len(no) == 1, (
                    ...
                    rects = [
                        i
                        for i in page.get_drawings_in_bbox((l, t, r, b))
                    ...
                    if temp.empty:
                        warnings.warn(
                            f"Driver {driver}, car No. {car_no}, has no lap at all on "
            ...
            # side by side
                ...
                bottom_pos = bottom_pos[
                    1:
                ]  # The first "NO TIME" is not the bottom of any table
                    ...
                    for col in range(3):
                        ...
                        driver = page.get_text(
                            ...
                            clip=(
                                col
                                * w
                                / 3,  # Each driver occupies ~1/3 of the page width
                                top_pos[row]
                                - 30,  # Driver name is ~20-30 px above the table
                                (col + 1) * w / 3,
                                top_pos[row] - 10,
                            ),
                        ).strip()
                        ...
                        lines = [
                            i
                            for i in page.get_drawings_in_bbox(bbox)
                            if np.isclose(i["rect"].y0, i["rect"].y1, atol=1)
                            and i["fill"] is None
                        ]
                        assert len(lines) >= 1, (
    ...
    def _parse_lap_times(self) -> pd.DataFrame:
        ...
        lap_1 = (
            self._parse_history_chart()[["car_no", "lap", "time"]]
        ...
        df = df.merge(
            lap_1, on=["car_no", "lap"], how="outer", indicator=True, validate="1:1"
        )
        ...
        df = df.merge(
            positions, on=["car_no", "lap"], how="outer", indicator=True, validate="1:1"
        )
        assert (df._merge == "both").all(), (
            f"Some laps only found in only one of "
        ...
        temp = self.classification_df[
            ["car_no", "fastest_lap_time", "fastest_lap_no", "laps_completed"]
        ]
        ...
        def to_json() -> list[dict]:
            ...
            return temp.apply(
                lambda x: LapImport(
                    object_type="Lap", foreign_keys=x.session_entry, objects=x.lap
                ).model_dump(exclude_unset=True),
                axis=1,
            ).tolist()
...
class QualifyingParser(BaseParser):
    ...
    def lap_times_df(self) -> pd.DataFrame:
        if self.is_pdf_complete is False:
            warnings.warn(
                "Lap times PDF is missing. Can get fastest laps only from the "
                "classification PDF"
            )
            df = self._apply_fallback_fastest_laps(
                pd.DataFrame(columns=["car_no"], data=[]),
    ...
    def _parse_classification(self):
        ...
        if not white_strips:
            raise ValueError(
                f'Could not find "NOT CLASSIFIED - " or "POLE POSITION LAP" '
        ...
        headers = page.get_text(
            "text", clip=(0, y, w, b_header)
        ).split()  # Col. headers/names
        ...
        for col_name in headers:
            ...
            if col_name not in ["LAPS", "TIME"]:
                assert len(col) == 1, (
                    f'Expected exactly one "{col_name}" in the header row in '
        ...
        if (
            "%" in headers
        ):  # Some PDFs may have one additional col. for Q1 cutoff percentage
            vlines.insert(
                headers.index("%") + 2,
        ...
        df["original_order"] = range(
            1, len(df) + 1
        )  # Driver's original order in the PDF
        ...
        if not_classified:
            ...
            pixmap = page.get_pixmap(
                clip=(0, not_classified[0].y1 + 1, w, page.bound()[3])
            )
            l, t, r, b = pixmap.x, pixmap.y, pixmap.x + pixmap.w, pixmap.y + pixmap.h
            pixmap = np.ndarray(
                [b - t, r - l, 3], dtype=np.uint8, buffer=pixmap.samples
            ...
            not_classified["finishing_status"] = (
                11  # TODO: should clean up the code later
            )
            not_classified.columns = df.columns.drop(
                ["original_order", "is_classified"]
            )
            not_classified = not_classified[
                (not_classified.NO != "") | not_classified.NO.isna()
            ]
            n = len(df)
        ...
        if disqualified:
            ...
            if np.isclose((disqualified.x0 + disqualified.x1) / 2, w / 2, rtol=0.1):
                # Go through the pixmap and find a wide white strip with 10+ px height
                pixmap = page.get_pixmap(
                    clip=(0, disqualified.y1 + 1, w, page.bound()[3])
                )
                l, t, r, b = (
                    pixmap.x,
                    pixmap.y,
                ...
                disqualified["finishing_status"] = (
                    20  # TODO: should clean up the code later
                )
                disqualified.columns = df.columns.drop(
                    ["original_order", "is_classified"]
                )
                disqualified = disqualified[
                    (disqualified.NO != "") | disqualified.NO.isna()
                ]
                n = len(df)
        ...
        df.temp = df.temp.astype(float)
        df.loc[df.position != "DQ", "temp"] = (
            df.loc[df.position != "DQ", "temp"].ffill()
        ...
        def to_json() -> list[dict]:
            ...
            for q in [1, 2, 3]:
                ...
                temp = pd.concat(
                    [
                        df[df.original_order <= n_drivers],
                ...
                """
                temp.loc[
                    (temp.finishing_status != 0) & temp.is_classified, "is_classified"
                ] = False
                """
                ...
                temp = temp.sort_values(
                    by=["is_dsq", "is_dnq", f"Q{q}", "original_order"]
                )
    ...
    @staticmethod
    def _assign_session_to_lap(
        classification: pd.DataFrame, lap_times: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        classification.Q2_LAPS = (
            classification.Q2_LAPS.astype(float) + classification.Q1_LAPS
        )
        lap_times = lap_times.merge(
            classification[["NO", "Q1_LAPS", "Q2_LAPS"]],
            left_on="car_no",
            right_on="NO",
            how="left",
        )
    ...
    def _parse_lap_times(self) -> pd.DataFrame:
                ...
                for col in range(3):
                    ...
                    driver = page.get_text(
                        ...
                        clip=(
                            ...
                            top_pos[row]
                            - 30,  # Driver name is usually 20-30 px above the table
                            (col + 1) * w / 3,
                            top_pos[row] - 10,
                        ),
                    ).strip()
                    if (
                        not driver
                    ):  # In the very last row may not have all three drivers. E.g., 20
                    ...
                    for line in top_lines:
                        no = page.search_for(
                            "NO", clip=(line[0], line[1] - 15, line[2], line[3])
                        )
                        assert len(no) == 1, (
                            f'Expected exactly one "NO" above the top line at '
                            f"({line[0], line[1], line[2], line[3]}) on p.{page.number} in "
        ...
        df = df.fillna({"lap_time_deleted": False}).rename(
            columns={0: "lap_no", 1: "pit", 2: "lap_time"}
        )
    ...
    def _apply_fallback_fastest_laps(
        self, lap_times_df: pd.DataFrame, drivers_with_invalid_fastest_laps: set[int]
    ) -> pd.DataFrame:
        """
        ...
        valid_laps = lap_times_df[
            ~lap_times_df.car_no.isin(drivers_with_invalid_fastest_laps)
        ]
...
class PitStopParser(BaseParser):
    ...
    def _parse(self) -> pd.DataFrame:
        ...
        for (
            page
        ) in doc:  # Can have multiple pages, though usually only one. E.g., 2023 Dutch
            # Get the position of the table
            page = Page(page)  # noqa: PLW2901
            driver = page.search_for("DRIVER")
            assert len(driver) == 1, (
                f'Expected exactly one "DRIVER" in {self.file}. Found: {driver}'
            )
            driver = driver[0]
            ...
            assert len(no) == 1, (
                f'Expected exactly one "NO" in the header row in {self.file}. '
                f"Found: {no}"
            )
            no = no[0]
            ...
            for col_name in headers:
                ...
                assert len(col) == 1, (
                    f'Expected exactly one "{col}" in the header row in '
        ...
        # Clean up the table
        df = pd.concat(df, ignore_index=True).dropna(
            subset="NO"
        ...
        def to_json() -> list[dict]:
            ...
            return pit_stop.apply(
                lambda x: PitStopData(
                    object_type="PitStop", foreign_keys=x.entry, objects=[x.pit_stop]
                ).model_dump(exclude_unset=True),
                axis=1,
            ).tolist()
