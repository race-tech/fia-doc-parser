...
def duration_to_millisecond(s: str | None) -> dict[str, str | int] | None:
    ...
    match s.count(":"):
        ...
        case 2:  # 1:23:45.678
            if m := re.match(
                r"(?P<hour>\d+):(?P<minute>\d+):(?P<sec>\d+)\.(?P<millisec>\d+)", s
            ):
...
class Page:
    def __init__(self, page: pymupdf.Page):
        ...
        self.header_min_height = (
            10  # The header image should have a height between 10 and 50px
        )
    ...
    def get_drawings_in_bbox(
        self, bbox: tuple[float, float, float, float], tol: float = 1
    ) -> list:
        ...
        for drawing in self.drawings:
            ...
            if (
                rect.y0 >= bbox[1] - tol
                and rect.y1 <= bbox[3] + tol
                and rect.x0 >= bbox[0] - tol
                and rect.x1 <= bbox[2] + tol
            ):
    ...
    def get_strikeout_text(self) -> list[tuple[pymupdf.Rect, str]]:
        ...
        paths = (
            self.drawings
        )  # Strikeout lines are in fact vector graphics. To be more precise,
        for path in paths:  # they are short rectangles with very small height
            for item in path["items"]:
                if (
                    item[0] == "re"
                ):  # If a graphic is a rect., check its height: absolute height
                    rect = item[
                        1
                    ]  # should < 1px, and have some sizable width relative to height
        ...
        for rect in lines:
            for w in words:  # `w` is a iterable `(x0, y0, x1, y1, text)`
                ...
                if text_rect.intersects(
                    rect
                ):  # If the word's location intersects with a
                    strikeout.append(
                        (rect, w[4])
                    )  # strikeout line, it's a strikeout text
        return strikeout

    def parse_table_by_grid(
        self, vlines: list[tuple[float, float]], hlines: list[tuple[float, float]]
